...
class TextTrainer:
    ...
    No window aggregation needed; predictions are directly participant predictions.

    Key differences from AudioTrainer:
    - forward() receives (token_ids, attention_mask, targets[, pids])
    - No participant aggregation loop during validation
    - Still accumulates pid→probs for fusion export
    """
    ...
    def __init__(
        ...
        logger.info(f"TextTrainer ready on {self.device}")
    ...
    def _run_epoch(
        ...
        """
        Participant-level forward/backward for text.
        Each batch item is one participant's full token sequence.
        """
        ...
        with torch.set_grad_enabled(is_train):
            for batch_idx, batch in enumerate(dataloader):
                # Batch format: (token_ids, attention_mask, targets, pids) or (token_ids, attention_mask, targets)
                if len(batch) == 4:
    ...
    def fit(
        ...
        logger.info(f"TextTrainer.fit: {epochs} epochs on {self.device}, fold={fold}")
        no_improve_count = 0
        ...
        for epoch in range(start_epoch, epochs + 1):
            ...
            log_line = (
                f"[Text Fold {fold}] Epoch [{epoch}/{epochs}] LR: {lr_current:.2e} | "
